...
const Json1 = (props) => {
  let config = {
      ...
      <View style={styles.container1}> 
          ...
          <Text style={styles.count}>{props.patientList["cin"]}</Text>
          </View>
          <View style={styles.card} >
          <Text style={styles.name}>matricule</Text>
                  <Text style={styles.count}>{props.patientList["matricule"]}</Text>
          </View>
                  {Object.keys(test).map((setNom, key) => (
                    
                      <View style={styles.card} >
              <Text style={styles.name}>{setNom}</Text>
                <Text style={styles.count}>{String(test[setNom])}</Text>
            </View>

        ))
}
</View><FormButton title="Suivant" onPress={() => { props.navigation.navigate("Json2") }} />
</ScrollView></View>
